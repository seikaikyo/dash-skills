...
The markdown report is always generated. Read `.github/prompts/security-audit-references/features-extended.md` for the SARIF/JSON schema details.
...
Read `.github/prompts/security-audit-references/features-extended.md` for baseline format and fingerprint calculation details.
...
Read `.github/prompts/security-audit-references/features-extended.md` for matching logic and report section format.
...
Pack files are in `.github/prompts/security-audit-references/packs/`. They follow the same format as custom checks.
...
All reference files are in `.github/prompts/security-audit-references/`.

For custom checks, read all `.md` files from this folder if it exists:
1. `.github/security-audit-custom/` (project-level custom checks, in the project root)
...
4. **Multi-framework detection** - Detect all frameworks in the project (not just the first match). Read up to 3 matching framework reference files from `.github/prompts/security-audit-references/frameworks/`. List all detected frameworks in the report Methodology section
...
9. Load custom checks - read all `.md` files from `.github/security-audit-custom/` if the folder exists. Treat each file as an additional checklist to run during Phase 2
10. **Load pack checks** - if `--pack` flags are set, read the matching pack files from `.github/prompts/security-audit-references/packs/`. Treat each pack as an additional checklist to run during Phase 2
...
For `full`, `diff`, `recheck` and `phase:2` modes: read `.github/prompts/security-audit-references/attack-vectors.md` for the detailed checklist.
...
3. Read `.github/prompts/security-audit-references/features-extended.md` for the triage flow specification.
...
1. Read `.github/prompts/security-audit-references/features-extended.md` for baseline format
...
1. Read `.github/prompts/security-audit-references/features-extended.md` for the schema specification
...
1. Read `.github/prompts/security-audit-references/features-extended.md` for matching logic
