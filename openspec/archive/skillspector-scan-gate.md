...
- 絕對分數在這個 corpus 上不可用：2026-08-27 基線日 50 目錄中 28 個被判 DO_NOT_INSTALL（含 anthropics 官方 docx / pptx / xlsx 全部 score 90 以上），靜態 pattern 對「帶腳本的 skill」一律重判。防護模型因此是 diff 制：全量人工抽查後產 per-dir baseline（`skillspector baseline`，存 `security-reports/baselines/<name>.yaml`），每日掃描帶 `--baseline` 只報基線外的新發現，新告警才是真訊號。基線日抽查結論：四個最重目錄（cybersecurity 1565 issues、trailofbits-curated 869、sentry-security-review 229、remotion 26）的 CRITICAL 項全部與該集合用途相符（資安教學文件裡的攻擊範例、SIEM/TTS 整合腳本的正當 API 呼叫），無內容與用途不符的注入跡象。
